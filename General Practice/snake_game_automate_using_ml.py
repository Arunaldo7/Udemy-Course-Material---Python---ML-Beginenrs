...
def capture_game_state(current_direction):
    game_state_dict = {};
...
player_stats_file = current_directory_path + "ai_player_stats.pkl"
...
try:
    ...
    col_transformer = pickle.load(open(current_directory_path + "col_transformer.pkl", "rb"));
    col_transformer_features = pickle.load(open(current_directory_path + "col_transformer_features.pkl", "rb"));
    model = pickle.load(open(current_directory_path + "model.pkl", "rb"));
    ...
    while RUN:
        ...
        if (not GAME_PAUSED) and (not GAME_STOPPED) :
            predict_dataset_dict = capture_game_state(player.direction);
            
            predict_dataset_dict = [predict_dataset_dict];
            
            # print("predict_dataset_dict:",predict_dataset_dict)
            
            predict_dataset = pd.DataFrame(predict_dataset_dict);
            
            encoded_independent_dataset = np.array(col_transformer.transform(predict_dataset));
            
            encoded_independent_dataset = pd.DataFrame(encoded_independent_dataset, columns = col_transformer_features);
            
            predicted_action = model.predict(encoded_independent_dataset.values)[0];
            
            print("predicted_action:",predicted_action);
            
            if predicted_action == "up":
                go_up();
